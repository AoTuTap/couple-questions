...
const QUESTIONS = [
  ...
  { text: "Tính cách, lối sống của hai chúng ta đang bù trừ cho nhau hay đang giống nhau?", depth: 3, type: "Thấu hiểu", icon: "☯️" },
  ...
  { text: "Nếu khi yêu nhau bạn nhận ra đối phương không phải người mình muốn cưới thì bạn sẽ làm gì?", depth: 5, type: "Câu sâu", icon: "🌊" },
  { text: "Nếu mình cãi nhau thì bạn sẽ làm gì?", depth: 4, type: "Xung đột", icon: "🤍" },
  { text: "Có bài học tình cảm nào ở quá khứ mà bạn muốn chia sẻ không?", depth: 5, type: "Quá khứ", icon: "📖" },
  { text: "Điều gì trong cuộc sống sẽ dễ khiến bạn áp lực nhất?", depth: 4, type: "Áp lực", icon: "☁️" },
  { text: "Kể ra 1 điểm tốt của tất cả những người bạn thân của bạn.", depth: 2, type: "Bạn bè", icon: "👥" },
...
function pickQuestion(total, used, current) {
...
function ActionButton({ children, onClick, disabled, variant = "dark", theme }) {
  const style = variant === "accent" ? `${theme.button} text-white` : variant === "outline" ? "border border-slate-200 bg-white/75 text-slate-800 hover:bg-white" : "bg-slate-950 text-white hover:bg-slate-800";
  return (
    <button type="button" onClick={onClick} disabled={disabled} className={`inline-flex min-h-12 items-center justify-center rounded-2xl px-5 py-3 text-sm font-semibold shadow-sm transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:translate-y-0 ${style}`}>
      {children}
    </button>
  );
}
...
function Center({ icon, title, desc, theme }) {
  return (
    <div className="enter relative text-center">
      <div className={`mx-auto mb-5 grid h-24 w-24 place-items-center rounded-[2rem] text-5xl shadow-inner ${theme.soft}`}>{icon}</div>
      <p className="text-2xl font-bold leading-relaxed md:text-4xl">{title}</p>
      <p className="mx-auto mt-4 max-w-xl text-base leading-7 opacity-65">{desc}</p>
    </div>
  );
}

function DepthMeter({ depth, info }) {
  return (
    <div className="rounded-2xl border border-white/35 bg-white/35 p-3 backdrop-blur">
      <div className="mb-2 flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-[0.18em] opacity-70"><span>Độ sâu</span><span>{depth}/5</span></div>
      <div className="mb-2 grid grid-cols-5 gap-1.5">{Array.from({ length: 5 }, (_, i) => <div key={i} className={`h-2.5 rounded-full ${i < depth ? info.bar : "bg-white/45"}`} />)}</div>
...
export default function CoupleQuestionsApp() {
  ...
  const card = index === null ? null : QUESTIONS[index];
  const depth = DEPTH[card?.depth || 1];
  const timerPercent = (seconds / TOTAL_SECONDS) * 100;
  const progress = (used.length / QUESTIONS.length) * 100;
  const danger = opened && seconds <= 10;
  ...
  useEffect(() => {
    if (!running || !opened || seconds <= 0) return undefined;
    const id = window.setInterval(() => setSeconds((value) => Math.max(0, value - 1)), 1000);
    return () => window.clearInterval(id);
  }, [running, opened, seconds]);

  useEffect(() => {
    if (seconds === 0) setRunning(false);
    if (sound && running && opened && seconds <= 10 && seconds > 0 && lastTickRef.current !== seconds) {
      lastTickRef.current = seconds;
      beep(520, 0.04, 0, 0.03);
    }
  }, [seconds, running, opened, sound]);
  ...
  function drawCard() {
    ...
    timeoutRef.current = window.setTimeout(() => {
      const next = pickQuestion(QUESTIONS.length, used, index);
      setIndex(next);
  ...
  function handleCardClick() {
    if (!revealed) return drawCard();
    if (!opened) return openCard();
    return drawCard();
  }
  ...
  return (
    <main className={`min-h-screen overflow-hidden px-4 py-8 transition-colors duration-500 ${theme.bg}`}>
      <style>{`
        ...
        @keyframes ring{50%{filter:drop-shadow(0 0 14px rgba(244,63,94,.4))}}
        @keyframes beat{35%{transform:scale(1.08)}65%{transform:scale(.98)}}
        ...
        .enter{animation:enter .5s ease both}.flip{animation:flip .58s ease both;transform-style:preserve-3d}.floating-heart{animation:floatHeart 1.25s ease-out forwards}.timer-ring{animation:ring 2s ease-in-out infinite}.danger{animation:beat .8s ease-in-out infinite}.shimmer::after{content:"";position:absolute;inset:0;transform:translateX(-130%);background:linear-gradient(90deg,transparent,rgba(255,255,255,.62),transparent);animation:shimmer 2.3s ease-in-out infinite}.shuffle-card{animation:shuffle .82s ease-in-out both;--x:90px;--r:18deg}.shuffle-card:nth-child(2){--x:-72px;--r:-14deg}.shuffle-card:nth-child(3){--x:38px;--r:8deg}.shuffle-card:nth-child(4){--x:-112px;--r:-22deg}.shuffle-card:nth-child(5){--x:128px;--r:24deg}.seal{animation:seal .55s ease both}@media (prefers-reduced-motion:reduce){.enter,.flip,.floating-heart,.timer-ring,.danger,.shimmer::after,.shuffle-card,.seal{animation:none!important}}
      `}</style>
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl flex-col justify-center">
        <header className="enter mb-6 text-center">
          ...
          <div className="mb-4 flex justify-center"><button type="button" onClick={() => { setSound((value) => !value); window.setTimeout(() => getAudioContext()?.resume?.(), 0); }} className="rounded-full border border-white/30 bg-white/20 px-4 py-2 text-sm font-semibold shadow-sm backdrop-blur hover:bg-white/30">{sound ? "🔊 Âm thanh bật" : "🔇 Âm thanh tắt"}</button></div>
        ...
        <div className="mb-6 rounded-[1.75rem] border border-white/30 bg-white/20 p-3 shadow-sm backdrop-blur">
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">{THEMES.map((item) => <button key={item.id} type="button" onClick={() => setThemeId(item.id)} className={`rounded-2xl border px-3 py-3 text-left transition hover:-translate-y-0.5 ${themeId === item.id ? "border-white bg-white/85 text-slate-950 shadow-md" : "border-white/25 bg-white/15 text-current hover:bg-white/25"}`}><div className="flex items-center gap-2 text-sm font-bold"><span className="text-lg">{item.icon}</span>{item.name}</div></button>)}</div>
        </div>
        ...
        <section className="rounded-[2rem] border border-white/70 bg-white/75 shadow-2xl shadow-rose-100/50 backdrop-blur-xl">
          <div className="p-5 md:p-8">
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div><p className="text-sm font-semibold uppercase tracking-[0.25em] opacity-75">Đã bốc {used.length} / {QUESTIONS.length} câu</p><div className="mt-3 h-2 w-56 overflow-hidden rounded-full bg-white/30"><div className={`h-full rounded-full transition-all duration-500 ${theme.accent}`} style={{ width: `${progress}%` }} /></div></div>
            ...
            {completed && <div className="enter mb-6 rounded-3xl border border-rose-200 bg-gradient-to-r from-rose-50 to-sky-50 px-5 py-4 text-center text-rose-700 shadow-sm"><div className="text-2xl">💕 ✨ 💌 🌸 💕</div><p className="mt-2 font-semibold">Hai bạn đã bốc hết 50 lá bài rồi.</p></div>}
            {shuffling && <div className="enter mb-6 rounded-[2rem] border border-white/35 bg-white/25 p-5 shadow-sm backdrop-blur"><p className="mb-4 text-center text-sm font-bold uppercase tracking-[0.24em] opacity-70">Đang xáo bài</p><div className="relative mx-auto h-32 max-w-md overflow-hidden rounded-[1.5rem]">{SHUFFLE_CARDS.map((item, i) => <div key={`${item}-${i}`} className={`shuffle-card absolute left-1/2 top-4 grid h-24 w-16 place-items-center rounded-2xl border border-white/70 bg-gradient-to-br from-white to-rose-50 text-3xl shadow-lg ${theme.soft}`} style={{ animationDelay: `${i * 0.08}s` }}>{item}</div>)}</div></div>}

            <div className="timer-ring relative mb-8 rounded-[2.35rem] p-[5px]" style={opened ? { background: `conic-gradient(${danger ? "#f87171" : theme.ring} ${timerPercent}%, rgba(255,255,255,.32) 0)` } : undefined}>
              <button type="button" onClick={handleCardClick} disabled={shuffling} className={`flip group relative w-full overflow-hidden rounded-[2rem] border bg-gradient-to-br p-7 text-left shadow-inner transition hover:-translate-y-1 hover:shadow-xl active:translate-y-0 md:p-10 ${revealed ? depth.card : "from-white via-rose-50 to-sky-50 border-white/70"} ${!revealed || !opened ? "shimmer" : ""}`}>
                <div key={burst} className="pointer-events-none absolute inset-0 overflow-hidden">{FLOATING.map((item, i) => <span key={`${item}-${i}`} className="floating-heart absolute bottom-14 text-2xl" style={{ left: `${16 + i * 13}%`, animationDelay: `${i * 0.08}s` }}>{item}</span>)}</div>
